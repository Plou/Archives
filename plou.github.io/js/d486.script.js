...
(function() {
  $(function() {
    var activate, clear, getActive, getSelected, goDown, goFirst, goLast, goUp, triggerItem, type;
    $.fn.finder = function(options) {
      var $items, $this, o;
      if (typeof options === "object") {
        o = $.extend({}, $.fn.finder.defaults, options);
      } else {
        o = $.fn.finder.defaults;
      }
      $items = $("." + o["class"]).addClass("on");
      $this = $(this);
      $this.on("click", function() {
        return $items.addClass("on");
      });
    ...
    clear = function(o) {
      $(this).val('');
      $(this).trigger("keyup");
      return $("." + o["class"]).removeClass("active");
    };
    goFirst = function(o) {
      return activate($("." + o["class"] + ".on:first"), o);
    };
    goLast = function(o) {
      return activate($("." + o["class"] + ".on:last"), o);
    };
    goDown = function(o) {
      var $next;
      $next = getActive(o).nextAll(".on").first();
      if ($next.html()) {
        return activate($next, o);
      } else {
        return goFirst(o);
      }
    };
    goUp = function(o) {
      var $prev;
      $prev = getActive(o).prevAll(".on").first();
      if ($prev.html()) {
        return activate($prev, o);
      } else {
        return goLast(o);
      }
    };
    getActive = function(o) {
      return $("." + o["class"] + ".active");
    };
    activate = function($item, o) {
      $item.addClass("active");
      return $("." + o["class"]).not($item).removeClass("active");
    };
    getSelected = function(o) {
      return $("." + o["class"] + ".selected");
    };
    triggerItem = function(o) {
      var $selectedItem;
      $selectedItem = $(getActive(o));
      $("." + o["class"]).not($selectedItem).removeClass("selected");
      return $selectedItem.trigger("click");
    };
    type = function(value, $items, o) {
      var i, searchRegEx, splitRegEx, valueArray, valueArrayLength, _results;
      splitRegEx = new RegExp("[\\s|,|;]+", "gi");
      value = value.replace(splitRegEx, ",");
      valueArray = value.split(",");
      valueArrayLength = valueArray.length;
      i = 0;
      _results = [];
      while (i < valueArray.length) {
        searchRegEx = new RegExp("[\\w+#+:+@]*" + valueArray[i].split("").join("[\\w+#+:+@]*") + "[\\w+#+:+@]*", "gi");
        $(this).resultCount = 0;
        $items.each(function() {
          if ($(this).attr("data-tags") && $(this).attr("data-tags").search(searchRegEx) >= 0) {
            $(this).addClass("on");
...
/*
//@ sourceMappingURL=jquery.finder.map
*/
...
(function() {
  ...
  Navigation = (function() {
    function Navigation(id, links, EVENT) {
      var _this = this;
      this.id = id;
      this.links = links;
      ...
      $(window).on('popstate', function(e) {
        return _this.getPost(e.target.document.location.href, false);
      });
    }

    Navigation.prototype.getPost = function(url, push) {
      var content,
        _this = this;
      content = url;
      return jQuery.ajax({
        ...
        success: function(data) {
          _this.displayPost(data);
          _this.pushState(url, data, push);
          return _this.el.trigger('change');
        },
        error: function(data) {
          return console.log('<p class="ajaxError">Error :' + data.status + ' - ' + data.statusText + '/>');
        }
      });
    };

    Navigation.prototype.displayPost = function(content) {
      return this.el.html($(content).find('#' + this.id).contents());
    };

    Navigation.prototype.pushState = function(url, content, push) {
      var title;
    ...
    Navigation.prototype.setGetEVENT = function(event) {
      if (event == null) {
        event = this.EVENT.get;
      }
      this.links.unbind(this.EVENT.get);
      this.EVENT.get = event;
      return this.bindGetEVENT();
    };

    Navigation.prototype.bindGetEVENT = function() {
      var _this = this;
      return this.links.on(this.EVENT.get, function(e) {
        e.preventDefault();
        _this.links.closest(".post").removeClass("current");
        $(e.target).closest(".post").addClass("current");
        return _this.getPost($(e.target).closest(".links").attr('href'), true);
      });
    };
...
(function() {
  $(function() {
    return $(window).ready(function() {
      var cList, cNav, cPost, navigation, setView, vList, vNav, vPost;
      $("[data-url]").on("click", function(e) {
        e.preventDefault();
        return document.location.href = $(this).attr("data-url");
      }).on("keyUp", function(e) {
        if (e.keyCode === 13) {
          return document.location.href = $(this).attr("data-url");
        }
      });
      $("[data-include]").each(function(e) {
        var $that;
        $that = $(this);
        return $.get($(this).attr("data-include"), null, function(data) {
          var $content;
      ...
      $(window).on("keyup", function(e) {
        switch (e.keyCode) {
          case [83, 70]:
            setView("nav");
            return $('#search').trigger("focus");
          case 27:
            setView("post");
            return $('a:focus').trigger("blur");
        }
      });
      $("nav .all").addClass("selected");
      $("nav:not(.navView nav)").on("mouseenter", function() {
        return setView("nav");
      });
      $("nav a").on("click", function(e) {
        e.preventDefault();
        setView("list");
        if ($(this).prop("selected") && !$(this).hasClass("all")) {
          $(this).prop("selected", false).removeClass("selected");
          return $('.all').prop("selected", true).addClass("selected");
        } else {
          $(this).siblings("a").prop("selected", false).removeClass("selected");
          return $(this).prop("selected", true).addClass("selected");
        }
      }).on("mouseenter focus", function() {
        var filter;
        filter = "." + $(this).attr("data-filter");
        $(".posts article").not(filter).addClass("hide");
        return $(".posts article" + filter).removeClass("hide");
      }).on("focus", function() {
        return setView("nav");
      });
      $("#post:not(.postView #post)").on("mouseenter", function() {
        if (!$("#search").is(':focus')) {
          return setView("post");
        }
      });
      ...
      setTimeout((function() {
        return $('#list').scrollTop(parseFloat($('#search').outerHeight()));
      }), 50);
      ...
      $('.posts .post').on('click', function(e) {
        e.preventDefault();
        return $(this).children('a').trigger('focus');
      });
      $("#list:not(.postView #list)").on("mouseenter", function() {
        return setView("list");
      });
      $("#list a").on("focus", function() {
        return setView("list");
      }).on("keyup", function(e) {
        switch (e.keyCode) {
          case 38:
            return $(this).closest('article').prev().children('a').trigger("focus");
          case 39:
            $(this).closest('article').addClass('active');
            return $(this).parent().siblings('article').removeClass('active');
          case 40:
            return $(this).closest('article').next().children('a').trigger("focus");
          case 37:
            return $(this).closest('article').removeClass('active');
        }
      });
      $(window).on("keyup", function(e) {
        switch (e.keyCode) {
      ...
      $(".no-touch nav a:last").on("blur", function() {
        return $(".no-touch nav").trigger('blur');
      });
      $(".no-touch nav").on("mouseleave blur", function() {
        var $current;
        $current = $(this).children(".selected");
        if ($current.hasClass('all')) {
          return $(".posts article").removeClass("hide");
        } else {
          return $current.trigger('mouseenter');
        }
      });
      Hammer(".touch section").on("swipeleft", function(e) {
        e.gesture.preventDefault();
        return setView("list");
      });
      Hammer(".touch section").on("swiperight", function(e) {
        e.gesture.preventDefault();
        return setView("nav");
      });
      Hammer(".touch #list").on("swiperight", function(e) {
        e.gesture.preventDefault();
        return setView("post");
      });
      Hammer(".touch nav").on("swipeleft", function(e) {
        e.gesture.preventDefault();
        return setView("post");
      });
      Hammer(".touch list").on("swipeleft", function(e) {
        e.gesture.preventDefault();
        return setView("post");
      });
        ...
        switch (view) {
          case "nav":
            if (!$("body > .page").hasClass("navView")) {
              ...
              return $("body > .page").removeClass("listView");
            }
            break;
          case "post":
          ...
          case "list":
            if (!$("body > .page").hasClass("listView")) {
              ...
              $("body > .page").removeClass("navView");
              return $("body > .page").removeClass("postView");
            }
        }
      };
      navigation = new Navigation("post", "links", {
        get: 'click focus'
      });
      cNav = new cManager.column("nav", 20, 250, 64);
      cPost = new cManager.column("post", 320, 696, 512);
      cList = new cManager.column("list", 168, 256);
      vPost = new cManager.view("post", [[cNav.max, cList.max, cPost.max], [cNav.medium, cList.max, cPost.max], [cNav.medium, cList.max, cPost.medium], [cNav.medium, cList.min, cPost.medium], [cNav.medium, "hide", cPost.max], [cNav.min, "hide", cPost.min], [cNav.min, "hide", 10]]);
      vNav = new cManager.view("nav", [[cNav.max, cList.max, cPost.max], [cNav.max, cList.max, cPost.medium], [cNav.max, cList.max, cPost.min], [cNav.max, cList.min, cPost.min], [cNav.max, cList.max], [cNav.max, cList.min], [cNav.max, "hide"]]);
      vList = new cManager.view("list", [[cNav.max, cList.max, cPost.max], [cNav.max, cList.max, cPost.medium], [cNav.max, cList.max, cPost.min], [cNav.max, cList.min, cPost.min], [cNav.max, cList.max], [cNav.medium, cList.max], [cNav.medium, cList.min], [cNav.min, cList.max], [cNav.min, cList.min]]);
      window.cM = new cManager.columnManager("cManager", [cNav, cList, cPost], [vNav, vPost, vList]);
      cM.el.one("updated", function() {
        return $(this).css('opacity', 1);
      });
      $(window).on("viewChange", function() {
        var _ref;
        switch (cM.view.id) {
          case "post":
            if (cM.view.pointer === 0) {
              return cNav.el.addClass('max');
            } else {
              return cNav.el.removeClass('max');
            }
            break;
          case "list":
            if ((_ref = cM.view.pointer) === 0 || _ref === 1 || _ref === 2 || _ref === 3 || _ref === 4) {
              return cNav.el.addClass('max');
            } else {
              return cNav.el.removeClass('max');
            }
